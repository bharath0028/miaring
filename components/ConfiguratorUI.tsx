...
export const ConfiguratorUI: React.FC<ConfiguratorUIProps> = React.memo(
  ({
    ...
    const wrapperClasses = isMobile
      ? `fixed left-0 right-0 bottom-0 z-50 transition-transform duration-300 ${isVisible ? 'pointer-events-auto translate-y-0 opacity-100' : 'pointer-events-none translate-y-full opacity-0'}`
      : `fixed left-1/2 bottom-6 transform -translate-x-1/2 z-50 transition-transform duration-300 ${isVisible ? 'pointer-events-auto translate-y-0 opacity-100' : 'pointer-events-none translate-y-full opacity-0'}`;
    ...
    const renderModeOptions = useMemo<ReadonlyArray<OptionItemData<"performance" | "quality">>>(() => [
      ...
      { value: "quality", label: "Fire", previewImage: "/assets/images/fire.png" },
    ], []);
                  ...
                  <div className="flex items-center justify-center gap-4">
                    {renderModeOptions.map((opt) => (
                        <Tooltip key={opt.value} content={opt.label}>
                          ...
                          >
                            {opt.previewImage ? (
                              <img src={opt.previewImage} alt={opt.label} className="w-full h-full object-cover rounded-full" draggable={false} />
                            ) : (
