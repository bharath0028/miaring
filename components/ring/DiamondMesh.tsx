...
export const DiamondMesh: React.FC<DiamondMeshProps> = ({
  ...
  const materialProps = useMemo(() => {
    const isQuality = renderMode === "quality" && !isMobile;
    const bounces = isQuality ? (isMacChrome ? 4 : 5) : isMacChrome ? 2 : 3;
    ...
    const aberrationStrength = isQuality
      ...
      ? 0.001
      : 0.002;
    const fastChroma = renderMode === "performance" || isMobile; // always fast on mobile
  ...
  }, [renderMode, isMacChrome, isMobile]);
